<x-app-layout>
    <div class="py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Header Section -->
            <div class="mb-10">
                <h1 class="text-4xl font-bold text-gray-900 mb-2">✨ Tambah Barang Baru</h1>
                <p class="text-gray-600 text-lg">Tambahkan barang impian Anda ke dalam wishlist</p>
            </div>

            <form action="/wishlist" method="POST" enctype="multipart/form-data" class="space-y-8">
                @csrf
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-3 gap-4">
                                <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 transition hover:bg-indigo-50"
                                    style="border-color: {{ old('prioritas') == 'Tinggi' ? '#4f46e5' : '' }};">
                                    <input type="radio" name="prioritas" value="Tinggi" class="w-4 h-4" required {{ old('prioritas') == 'Tinggi' ? 'checked' : '' }}>
                                    <span class="ml-3 font-semibold">🔴 Tinggi</span>
                                </label>
                                <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 transition hover:bg-indigo-50"
                                    style="border-color: {{ old('prioritas') == 'Sedang' ? '#4f46e5' : '' }};">
                                    <input type="radio" name="prioritas" value="Sedang" class="w-4 h-4" {{ old('prioritas') == 'Sedang' ? 'checked' : '' }}>
                                    <span class="ml-3 font-semibold">🟡 Sedang</span>
                                </label>
                                <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 transition hover:bg-indigo-50"
                                    style="border-color: {{ old('prioritas') == 'Rendah' ? '#4f46e5' : '' }};">
                                    <input type="radio" name="prioritas" value="Rendah" class="w-4 h-4" {{ old('prioritas') == 'Rendah' ? 'checked' : '' }}>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <label class="relative flex items-center p-4 border-2 border-gray-300 rounded-lg cursor-pointer hover:border-indigo-500 transition hover:bg-indigo-50">
                                    <input type="radio" name="status" value="0" class="w-4 h-4" checked>
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <textarea name="catatan" 
                                ...
                                rows="4"
                                class="w-full border-2 border-gray-300 rounded-lg px-4 py-3 focus:border-indigo-500 focus:outline-none transition duration-200 focus:ring-2 focus:ring-indigo-200">{{ old('catatan') }}</textarea>
                ...
                <div class="flex gap-4 justify-end">
                    ...
                    <button type="submit" class="px-8 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold rounded-lg shadow-lg transition duration-300 transform hover:scale-105 flex items-center gap-2">
                        <span>✏️ Simpan Barang</span>
                    </button>
                </div>
            </form>
        </div>
